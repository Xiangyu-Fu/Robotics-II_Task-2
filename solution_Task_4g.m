...
if(connected)
    ...
    % generate path
    target_pos_path = [];
    target_pos_path = path_generate(target_pos_path, 1, 0.1);                  
    target_pos_path = path_generate(target_pos_path, 1, 0.1); 
    target_pos_path = path_generate(target_pos_path, 0, 0.1); 
    target_pos_path = path_generate(target_pos_path, 0, 0.1);
    ...
    for n=1:size(target_pos_path)
        ...
        % loop
        while (error_sum > 0.1)
        % while(currentTime-startTime < 2) % this condition just used for debug
...
function [target_path] = path_generate(target_path, clockwise, precision)
    r=2.1;
    if(clockwise)
    ...
    Circle1=r*cos(theta);
    Circle2=r*sin(theta);
    generate_path = [Circle1;Circle2]';
    target_path = [target_path;generate_path];    
end